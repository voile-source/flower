  ...
  <div class="workspace">
    <div class="fl-main">
      ...
      <div v-if="listData.length != 0" class="fl-option fl-options">
        <File v-for="(item,index) in listData" 
        :key="'files'+item.id" 
        :item = "item" 
        :index="index" 
      ...
      <el-empty v-else  description="当前文件夹为空"></el-empty>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    init() {
      this.$apiFun.file.recentList().then(res => {
        this.listData = res.data.articleList.reverse();
      })
    }
  },
}
